...
print(f"K(x_a, x_b) = {kernel_aliased:.6f}   (should ALSO be close to 1.0, despite x_b looking very different)")
print()
...
def scale_to_range(X, feature_min, feature_max, target_min=0, target_max=np.pi):
    ...
    the TRAINING set) into [target_min, target_max].
    """
...
print(f"Out-of-range test point {x_test_extreme} scales to {np.round(x_test_extreme_scaled, 4)} -> falls OUTSIDE [0, pi].")
print()
